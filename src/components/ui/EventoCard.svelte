<script lang="ts">
  ...
  export let maxPartecipanti: number | undefined = undefined;
  export let organizzatore: { id: string; nome: string };
  export let state: 'bozza' | 'pubblicato' | 'concluso' | 'annullato' | undefined = undefined;
  export let partecipanti: Partecipante[] = [];
  
  // Debug log when props change
  $: {
    console.log('EventoCard props:', {
      id,
      titolo,
      data,
      luogo,
  ...
  function navigateTo(path: string) {
    console.log('Navigating to organizer profile:', path);
    push(path);
  }
  ...
  function getStatoBadgeClass(stato: string): string {
    switch (stato) {
      ...
      case 'concluso': return 'badge-info';
...
<div class="card bg-base-100 shadow-xl h-full cursor-pointer hover:shadow-2xl transition-all duration-300" 
  on:click={() => navigateTo(`/evento/${id}`)}
>
  <figure class="relative aspect-[16/9] overflow-hidden">
    <img 
      src={immagine || defaultImage} 
      alt={titolo} 
    ...
    {#if state}
      <div class="absolute top-2 right-2">
        <div class="badge badge-lg {getStatoBadgeClass(state)}">
          {formatStato(state)}
        </div>
      </div>
    {/if}
  ...
  <div class="card-body p-4">
    ...
    <div class="flex flex-col gap-2 mt-2">
      ...
      <div class="flex items-center gap-2 text-sm text-base-content/70">
        ...
        <span class="font-medium">{numeroPartecipanti} {maxPartecipanti ? `/ ${maxPartecipanti}` : ''} partecipanti</span>
      </div>
      ...
      <div class="flex items-center gap-2 text-sm text-base-content/70">
        ...
        {:else}
          <span class="truncate font-medium">Organizzato da {organizzatore?.nome || "Anonimo"}</span>
        {/if}
      </div>      
    </div>
  </div>
